...
We will use the term *callable object* rather than *function* in this description, except when refering to an actual C++ function.
A callable object may be an actual C++ functions, possibly one created by instantiating a function template.
A callable object may also be an instance of a class or struct that has a function call operator (`operator()(T...)`) implemented for some type, or sequence of types, `T`.

We want to work with callable types `F` such that if `f` is an object of type `F`, it supports a call like:
